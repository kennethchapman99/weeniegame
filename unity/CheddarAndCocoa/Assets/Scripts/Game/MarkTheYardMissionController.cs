...
namespace CheddarAndCocoa.Game
{
        ...
        IMissionSuccessPresentationController
    {
        ...
        public bool HandleBark(int dogIndex)
        {
            ...
            if (_state.Claimed <= 0) return false;
            if (_context.IndexOfDog(DogId.Cocoa) != dogIndex)
            {
                _context.SetCue("Cheddar's bark won't scare this squirrel off - Cocoa defends the marked zones.");
                _context.SetJuice(GameManager.JuiceFeedbackKind.WarningMiss, "COCOA: BARK THE THIEF!");
                _context.SpawnWorldPop(_context.Dogs[dogIndex].transform.position + Vector3.up, "COCOA'S JOB", new Color(1f, 0.72f, 0.25f));
                _context.MarkFailedInteraction(DogId.Cheddar, "Cheddar tried to bark off the marking-zone thief");
        ...
        private void ReclaimZone()
        {
            ...
            MissionPropArt.SetSprite(_zoneMarkers[target].GetComponent<MissionPropArtAttachment>(),
                FinalGameplayArt.MarkYardZoneStolen);
            _state.Unclaim();
        ...
        private void BuildScene()
        {
            ...
            for (int i = 0; i < _zones.Length; i++)
            {
                ...
                MissionPropArt.AttachPad(go, FinalGameplayArt.MarkYardZoneUnclaimed, 0.012f, 18);
                go.SetActive(false);
                _zoneMarkers[i] = go;
            }
            ...
            _squirrelArt = MissionPropArt.AttachObject(_squirrel, FinalGameplayArt.MarkYardSquirrelWatch, 0.014f, 24);
        ...
        private void SetZoneVisual(int i, bool claimed)
        {
            ...
            MissionPropArt.SetSprite(_zoneMarkers[i].GetComponent<MissionPropArtAttachment>(),
                claimed ? FinalGameplayArt.MarkYardZoneClaimed : FinalGameplayArt.MarkYardZoneUnclaimed);
        }
