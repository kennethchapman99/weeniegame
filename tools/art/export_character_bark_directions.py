#!/usr/bin/env python3
"""Extract diagonal and straight bark boards into normalized alpha frames."""
...
def extract(sheet_path):
    ...
    for row in range(2):
        for column in range(4):
            ...
            if bbox is None:
                raise RuntimeError(f"No bark sprite in {sheet_path.name} row {row} column {column}")
...
def main():
    ...
    for group, directions in GROUPS:
        for dog in ("cheddar", "cocoa"):
            cells = extract(SOURCE_ROOT / f"{dog}_bark_{group}_v01.png")
            ...
            for index, cell in enumerate(cells):
                ...
                output = OUTPUT_ROOT / dog.capitalize() / "Motion" / f"{dog}_bark_{directions[index // 4]}_{index % 4:02d}.png"
                output.parent.mkdir(parents=True, exist_ok=True)
                canvas.save(output, optimize=True)
                exported += 1
    print(f"Exported {exported} directional bark frames.")
