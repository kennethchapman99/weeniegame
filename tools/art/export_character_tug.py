#!/usr/bin/env python3
"""Extract three-frame east-facing tug strips into normalized alpha sprites."""
...
def main() -> None:
    for dog in ("cheddar", "cocoa"):
        with Image.open(SOURCE_ROOT / f"{dog}_tug_east_v01.png") as opened:
            sheet = opened.convert("RGBA")
        cells = []
        for column in range(3):
            cell = sheet.crop((round(column * sheet.width / 3) + 8, 8,
                               round((column + 1) * sheet.width / 3) - 8, sheet.height - 18))
            ...
            if bbox is None:
                raise RuntimeError(f"No tug sprite in {dog} column {column}")
        ...
        for frame, cell in enumerate(cells):
            ...
            output = OUTPUT_ROOT / dog.capitalize() / "Motion" / f"{dog}_tug_e_{frame:02d}.png"
            output.parent.mkdir(parents=True, exist_ok=True)
            canvas.save(output, optimize=True)
    print("Exported 6 tug frames.")
