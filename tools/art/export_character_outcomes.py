#!/usr/bin/env python3
"""Extract two-frame outcome loops into normalized alpha sprites."""
...
CLIPS = ("stunned", "rescued", "proud", "sad")


def main():
    for dog in ("cheddar", "cocoa"):
        with Image.open(SOURCE_ROOT / f"{dog}_outcomes_east_v01.png") as opened:
        ...
        for row in range(2):
            for column in range(4):
                ...
                if bbox is None:
                    raise RuntimeError(f"No outcome sprite in {dog} row {row} column {column}")
        ...
        for index, cell in enumerate(cells):
            ...
            output = OUTPUT_ROOT / dog.capitalize() / "Motion" / f"{dog}_{CLIPS[index // 2]}_e_{index % 2:02d}.png"
            output.parent.mkdir(parents=True, exist_ok=True)
            canvas.save(output, optimize=True)
    print("Exported 16 outcome frames.")
