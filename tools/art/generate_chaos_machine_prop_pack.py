...
def lever_base(draw: ImageDraw.ImageDraw, pulled: bool):
    shadow(draw, (94, 380, 418, 438), 34)
    plank(draw, (126, 332, 386, 380), "#77451f")
    gear(draw, 160, 304, 42, "#79d4e6" if pulled else "#d8b24c")
    gear(draw, 350, 304, 42, "#ffcf4f" if pulled else "#b78f37")
    draw.rounded_rectangle((206, 192, 306, 356), radius=22, fill=rgba("#5d3419"),
                           outline=rgba("#2c170b"), width=8)
    pivot = (256, 226)
    angle = -48 if pulled else 34
    rad = math.radians(angle)
    x1 = pivot[0] + math.cos(rad) * 156
    y1 = pivot[1] + math.sin(rad) * 156
    draw.line((pivot[0], pivot[1], x1, y1), fill=rgba("#3f2411"), width=22)
    draw.line((pivot[0], pivot[1], x1, y1), fill=rgba("#c96732"), width=14)
    draw.ellipse((pivot[0] - 32, pivot[1] - 32, pivot[0] + 32, pivot[1] + 32),
                 fill=rgba("#fff1a8"), outline=rgba("#3f2411"), width=7)
    draw.ellipse((x1 - 34, y1 - 34, x1 + 34, y1 + 34),
                 fill=rgba("#ff7a51" if pulled else "#ffcf4f"), outline=rgba("#3f2411"), width=7)
    if pulled:
        for angle in [-30, -8, 16, 38]:
            rad = math.radians(angle)
            draw.line((344, 156, 344 + math.cos(rad) * 74, 156 + math.sin(rad) * 54),
                      fill=rgba("#fff1a8", 190), width=8)
        dog_paw(draw, 118, 238, 0.78, rgba("#63f1e6", 210))
        dog_paw(draw, 396, 238, 0.78, rgba("#ffbf5a", 210))
    else:
        dog_paw(draw, 118, 308, 0.72, rgba("#63f1e6", 175))
        dog_paw(draw, 396, 308, 0.72, rgba("#ffbf5a", 175))


def lever_ready(draw: ImageDraw.ImageDraw):
    lever_base(draw, pulled=False)


def lever_running(draw: ImageDraw.ImageDraw):
    lever_base(draw, pulled=True)
...
DRAWERS = {
    "chaos_lever_ready": lever_ready,
    "chaos_lever_running": lever_running,
...
def write_readme():
    ...
    (REF_ROOT / "README.md").write_text(
        ...
        "lever ready/running state and the towel-drop, basket-tip, and toy-launch junctions while "
        "retaining controller-owned labels, range markers, and cascade timing. These are couch-test "
        "props, not a final hand-authored prop/animation pipeline.\n\n"
        f"{names}\n",
        encoding="utf-8",
    )
...
def write_contact_sheet(images: list[tuple[str, Image.Image]]):
    ...
    cell = 256
    cols = 5
    sheet = Image.new("RGBA", (cell * cols, cell), (245, 240, 226, 255))
