#!/usr/bin/env python3
"""Normalize generated storybook bark cutouts to the shared dog-motion canvas and baseline."""
...
def subject(path: Path) -> Image.Image:
    ...
    if bbox is None:
        raise RuntimeError(f"No opaque bark subject in {path}")
...
def main() -> None:
    ...
    for dog in ("cheddar", "cocoa"):
        ...
        paths = [folder / f"{dog}_bark_storybook_e_{frame:02d}.png" for frame in range(2)]
        ...
        for path, image in zip(paths, subjects):
            ...
            exported += 1
    print(f"Normalized {exported} storybook bark frames to {CANVAS[0]}x{CANVAS[1]}.")
