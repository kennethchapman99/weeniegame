...
> - **Baby Bird Bedlam** (Feast-and-Fend, *2026-07-12, first post-taxonomy primitive, built controller-native*): chicks tumble from the oak nest; Cheddar grabs each one and shake-shake-shakes it down in a cartoon gulp, but he's defenseless while his mouth is full — the parent birds dive at him, and only Cocoa's bark inside the dive window drives them off. An un-repelled dive pecks him loose (three pecks fail); an unclaimed chick gets airlifted home.
...
| `CoopFeastGuardPuzzle` | #1 busy-hands split + #7 timed defense | **The eater can't defend, the defender can't eat**: the prize is consumed in stages (grab, then repeated shakes) and dives only threaten a held prize; the partner must `Repel` each dive before its window expires | Expired dive = **Peck** (prize escapes, shake progress lost; capped fail); hesitating on a grounded prize = **Airlift** (mistake sting, no fail); the gulp shake mid-dive cancels the dive |
