...
> Status: all primitives **and** a position/input driver each landed, full PlayMode suite green. **All eight** co-op-puzzle beats are now **wired into real missions** (seven fresh missions plus a rescue-timing upgrade of an existing one), proving the end-to-end integration pattern: a primitive field on `GameManager`, a `BeginRound` setup block, a `Tick*` driving it from dog positions, `Force*` test hooks, per-dog guidance, and a `MissionDefinition`.
