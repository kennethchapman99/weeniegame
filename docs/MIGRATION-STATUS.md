...
| Snack Heist | `SnackHeistMissionController` | Owns Cheddar-only theft, Cocoa-only active-heist defense, required guard-before-final-stash gating, wrong-role recovery, held success, recovery/steal state, squirrel targeting/timing, failure, and snapshots; consumes the shared squirrel and treat pool through narrow context services. |
...
| Baby Bird Bedlam | `BabyBirdBedlamMissionController` | First mission built controller-native *after* the migration completed (2026-07-12, owner-requested). Owns the chick/nest actors, `CoopFeastGuardPuzzle` grab/shake/dive lifecycle, Cocoa's bark-repel via `HandleBark`, peck-cap failure, and airlift pressure. Promoted into the quality-ordered showcase five on 2026-07-15; mission seeds now use stable mission identity rather than selector position. |
