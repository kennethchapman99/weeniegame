...
- **Bait-and-switch** (`CoopBaitSwitchPuzzle`): an **enemy** (squirrel/coyote/vacuum) with three readable attention states — **guarding** the real prize (not committed), **committed** lunging at the decoy (the open window, driven by `Committed`), and a **wised-up bolt/startle** on a `Backfire`; a **decoy prop** (fake nut pile / noisy toy) the baiter feints at; a **commitment meter** that fills toward the band and visibly redlines into the backfire zone near full; the baiter's feint pose and the striker's snatch pop on a `Hit` vs a whiff puff when the enemy was still guarding.
- **Social manipulation** (`CoopSocialManipulationPuzzle`): a **human NPC** with reaction states — neutral, **getting it** (while `ExactMatch`, a thought-bubble of the right thing: a leash/walk), and **confused/misreading** (a "?" then bringing the wrong item) when `Confusion` maxes; **stimulus station props** per stimulus (door/leash/shoe/charger/hallway) with an "active" glow while a dog covers it; small per-stimulus dog poses (Cocoa's dignified door-stare, Cheddar presenting the leash).
